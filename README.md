# 抖音海外 IP 属地精确分流：视频优先直连，多端适配

**属地在海外，刷视频照样痛快。**

想挂海外属地，又不想刷个视频也绕远路？让关键接口走你选择的代理，视频、图片、直播资源尽量直连。已有节点就能试，不必把整个抖音塞进代理。

- **属地随节点，媒体少绕路。** 关键 API 保留代理，已验证的媒体资源优先直连。
- **给现有配置加一层。** 优先使用模块、插件和覆写；需要合并的客户端有单独教程。
- **不碰账号和内容。** 不需要账号密码、Cookie、HTTPS 解密证书，也不修改抖音 App。
